...
ws.createServer(function (conn) {
    ...
    conn.on("text", function (str) {
        ...
        if (json.command == 'login') {
            if (conn.auth) {
                conn.sendText(JSON.stringify({success: true, command: json.command, message: 'You are already logged in.'}));
            } else {
                if (json.password === config.password) {
                    console.log('[FPSmeter] Login successful.');
                    conn.sendText(JSON.stringify({success: true, command: json.command, message: 'Login was successful.'}));
                    conn.auth = true;
                } else {
                    console.log('[FPSmeter] Login failed. Wrong password.');
                    conn.sendText(JSON.stringify({success: false, command: json.command, message: 'Login failed. Wrong password.'}));
                }
            }
        } else {
            if (conn.auth) {
                if (json.command === 'serverinfo') {
                    ...
                    conn.sendText(JSON.stringify({success: true, command: json.command, currentFPS: currentFPS, currentPlayers: currentPlayers, maxFPS: maxFPS}));
                } else {
                    console.log('[FPSmeter] Unkwown command "' + json.command + '".');
                    conn.sendText(JSON.stringify({success: false, command: json.command, message: 'Unkwown command "' + json.command + '".'}));
                }
            } else {
                console.log('[FPSmeter] Please login first.');
                conn.sendText(JSON.stringify({success: false, command: json.command, message: 'Please login first.'}));
            }
        }
    });
